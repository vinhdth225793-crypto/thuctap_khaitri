                    ...
                    <tbody>
                        @forelse($hocViens as $index => $bghv)
                            <tr>
                                ...
                                <td><div class="d-flex align-items-center"><div class="avatar-mini rounded-circle bg-light border text-center me-2" style="width:35px;height:35px;line-height:35px;"><i class="fas fa-user text-muted"></i></div><div><div class="fw-bold text-dark">{{ $bghv->hocVien->ho_ten ?? 'N/A' }}</div><code class="smaller">#{{ $bghv->hocVien->ma_nguoi_dung ?? 'N/A' }}</code></div></div></td>
                                <td><div class="small"><i class="far fa-envelope me-1 text-muted"></i>{{ $bghv->hocVien->email ?? 'N/A' }}</div><div class="small mt-1"><i class="fas fa-phone-alt me-1 text-muted"></i>{{ $bghv->hocVien->so_dien_thoai ?? 'N/A' }}</div></td>
                                ...
                                <td class="pe-4 text-center">
                                    <div class="d-flex justify-content-center gap-1">
                                        <button type="button" class="btn btn-sm btn-outline-warning border-0 btn-edit-enroll" data-id="{{ $bghv->id }}" data-name="{{ $bghv->hocVien->ho_ten }}" data-date="{{ $bghv->ngay_tham_gia ? $bghv->ngay_tham_gia->format('Y-m-d') : '' }}" data-status="{{ $bghv->trang_thai }}" data-note="{{ $bghv->ghi_chu }}" title="Sửa ghi danh"><i class="fas fa-edit"></i></button>
