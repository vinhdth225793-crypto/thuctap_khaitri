...
<head>
    ...
    {{-- Favicon: dùng logo do admin upload (Settings → Liên hệ → Logo trang web), fallback default --}}
    @if(!empty($siteLogo ?? null))
        <link rel="icon" type="image/png" href="{{ asset($siteLogo) }}">
        <link rel="shortcut icon" type="image/png" href="{{ asset($siteLogo) }}">
        <link rel="apple-touch-icon" href="{{ asset($siteLogo) }}">
    @else
        <link rel="icon" type="image/svg+xml" href="{{ asset('images/default-course.svg') }}">
    @endif
        ...
        * {
        ...
        .auth-brand-card::before {
            ...
            background:
                ...
                linear-gradient(315deg, rgba(14, 165, 233, 0.34), transparent 48%);
            pointer-events: none;
        }
        ...
        .auth-form-card .vip-form-control,
        ...
        @media (max-width: 575.98px) {
            ...
            .auth-form-card .form-check.d-flex {
